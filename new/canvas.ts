class Canvas {
    ...
    static raw_choices: {choice_id: string, choice_text: string}[];
    static fa_number: any;
    ...
    static extractQuestion() {
        ...
        let questionTextElement = questionElement.querySelector('.question_text p') as HTMLElement;
    ...
    static extractChoices() {
        ...
        const result = Array.from(answerElements).map(answer => {
            const inputElement = answer.querySelector('input.question_input') as HTMLInputElement;
            
            const choice_id = inputElement.value;
            
            const labelElement = answer.querySelector('.answer_label') as HTMLElement;
